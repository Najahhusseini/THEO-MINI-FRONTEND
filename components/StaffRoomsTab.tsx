...
import { getMyAssignedRooms, updateCleaningTaskStatus, releaseRoom } from '@/lib/api'
...
export default function StaffMyRooms() {
    ...
    const [showReleaseModal, setShowReleaseModal] = useState<AssignedRoom | null>(null)

    const loadMyRooms = useCallback(async () => {
        if (!staff?.id) return
        try {
            const data = await getMyAssignedRooms()
            setRooms(data.assignedRooms || [])
        } catch (error) {
    ...
    useEffect(() => {
        loadMyRooms()
        const interval = setInterval(loadMyRooms, 30000)
        const handleRefresh = () => loadMyRooms()
    ...
    const handleCompleteCleaning = async (room: AssignedRoom) => {
        if (!room.cleaning_request_id) {
            toast.error('No cleaning request associated with this room')
            return
        }
        if (room.cleaning_status !== 'cleaning') {
            toast.error(`Cannot complete cleaning – room is ${room.cleaning_status}`)
            return
        }
        setUpdating(room.id)
        try {
    ...
    const handleReleaseRoom = async (room: AssignedRoom) => {
        setUpdating(room.id)
        try {
    ...
    if (loading) return <div className="flex justify-center items-center h-64 text-gray-500">Loading your rooms...</div>
    ...
    if (rooms.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <button onClick={loadMyRooms} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Refresh
                </button>
            </div>
        )
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="max-w-7xl mx-auto mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">
                    Rooms assigned to you by Head of Housekeeping. Click Release when asked to give up a room.
                </p>
            </div>
